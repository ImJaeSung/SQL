...
## < 최솟값 > 최댓값 ( 최댓값이 몇 개인지는 제가 확인 한 번 할 예정 mysql 조금씩 차이가 날 수 있어 체크 예정)
	
-- exists 문
# 조건의 결괏값이 있는 지 없는지를 확인해 1행이라도 있으면 TRUE, 없으면 FLASE를 반환
# where exists 문 사용하면-> 서브쿼리 결괏값이 1행이라도 있으면 TRUE 메인 주 쿼리 실행, 그 메인 쿼리에 작성된 전체 데이터를 검색하는 것
...
## inline view 인라인뷰 용어 
## inline view : 서브쿼리가 from 절에서 안에서 사용되는 경우
## inner join으로 비교해 보자!
...
select m.mem_name, m.mem_id, b.price, b.prod_name from member m
	inner join (select price, prod_name, mem_id from buy where price > 50) as b 
