...
-- 서브쿼리의 특징
## 1. 소괄호 안에 들어간다. ()
## 2. 메인 쿼리 실행 전에 한 번만 실행이 된다.
## 3. 비교 연산자에 서브쿼리 사용하려는 경우는 서브쿼리 오른쪽에 써야한다. 
...
## 단일행 서브쿼리 -> 하나의 값이 나오게 됨
## 다중 서브쿼리  -> 여러 개의 값이 나오게 됨

## where절에서 사용하는 쿼리는 중첩서브쿼리 라고 부른다.
## where 안에 들어가는 내용이기 때문에, 비교 연산자등과 함께 사용할 수 있다.
## 서브쿼리를 통해서 반환되는 결과가 단일, 다중에 따라서 문법도 달라진다.
...
## 다중값을 사용하게 된다면
select * from member
where mem_id in (select mem_id from member); # in은 다중값을 사용할 때 사용하는 문법
...
## 다중값을 만들어 보자!
select * from member
where mem_id not in (select mem_id from member where height > 163); # not을 붙이면 그 외에 값으로 출력이 된다.
