...
-- null 값이 존재 어떻게 해결하면 될까?
# isnull
# ifnull(na, 바꿀 문자) 
# 내가 원하는 na 값을 이렇게 바꿀 수 있다.
select isnull(phone1) from member;
select ifnull(phone1, '02') from member;

-- 문자열 다루는 함수 중 lower, upper : 대소문자
select lower(mem_id), mem_id from member;
select upper(mem_id), mem_id from member;

-- trim , rtrim ltrim : 문자열 공백제거 양쪽, 오른쪽 왼쪽` 
select ltrim('    abc');
select rtrim('abc     ') ;
...
-- 날짜 포맷 변경할 때 : date_format
select date_format(debut_date, '%Y%m%d') from member;

# (%Y, %y), (%M, %m), (%d, %D)는 다르고
# 원하는 데이터 포맷을 추출할 수 있다.
select date_format(debut_date, '%y%m%d') from member;
select date_format(debut_date, '%Y%M%D') from member;

-- 데이터 검증에 필요한 count 진행!
select count(*) 
	, count(phone1) # 특정 값을 count 할 때 "null 값이 제외"되고 카운팅이 된다.  
	from member; 
...
select count(customerNumber) from classicmodels.payments
	...
	having count(customerNumber)>1;  # group by 를 이용해서 데이터 검증할 때 간단하게 확인
